...
def optimize_schedule(config: SchedulingConfig) -> ScheduleResult:
    ...
    # 1. Minimize total shifts (efficiency)
    total_shifts = sum(employee_shift[emp.id][shift['id']] 
                      for emp in config.employees for shift in shifts)
    objectives.append(total_shifts)
